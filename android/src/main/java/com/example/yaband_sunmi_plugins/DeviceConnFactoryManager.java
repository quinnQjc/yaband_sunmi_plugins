...
public class DeviceConnFactoryManager {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case READ_DATA:
                    ...
                    String status = "R.string.strings.xml";
                    if (sendCommand == esc) {
                        //设置当前打印机模式为ESC模式
                        if (currentPrinterCommand == null) {
    ...
    private void sendStateBroadcast(int state) {
        ...
        intent.putExtra(DEVICE_ID, id);
        Log.i("getContext","App.getContext() == null = "+(App.getContext() == null));
        Log.i("getContext","MainActivity.getContext() == null = "+(YabandSunmiPluginsPlugin.getContext() == null));
        if (App.getContext() != null) {
            App.getContext().sendBroadcast(intent);
        }else {
            YabandSunmiPluginsPlugin.getContext().sendBroadcast(intent);
        }
    }
